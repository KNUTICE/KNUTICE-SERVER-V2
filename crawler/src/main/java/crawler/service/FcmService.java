...
public class FcmService {
    ...
    public void batchSend(FcmDto fcmDto, List<String> deviceTokenList) {
        ...
        if (tokenSize == 1) {
            ...
            log.info("[{}] 배치 처리 병렬 실행 시작 (토큰 집합 수: {})", Thread.currentThread().getName(),
                tokenSize);
    ...
    public void processBatch(List<MessageWithFcmToken> targetList, int retryCount) {
        ...
        while (batchResponse == null && attempt <= MAX_RETRIES) {
            try {
                ...
                if (errorCode == null) {
                    log.error("[{}] 에러 코드가 없는 FirebaseMessagingException 발생: {}", Thread.currentThread().getName(), e.getMessage());
                    return;
                }
                ...
                if (NOT_RETRYABLE_ERROR_CODES.contains(errorCode)) {
                    log.error("[{}] 메세지를 전송할 수 없는 상태입니다 (재시도 불가). 에러 코드: {}",
                        Thread.currentThread().getName(), errorCode);
                    return;
                } else if (RETRYABLE_ERROR_CODES.contains(errorCode)) {
                    // 재시도가 가능한 오류 처리
                    log.warn("[{}] 일시적인 오류 발생, 재시도 가능: {} - 시도 횟수: {}", Thread.currentThread().getName(), errorCode, attempt);
                    backOff(attempt++);  // 백오프 후 재시도
                }

                // 그 외 예상치 못한 오류는 로깅만 하고 종료
                log.error("[{}] 예상치 못한 오류가 발생했습니다: {} - {}번 시도", Thread.currentThread().getName(), errorCode, attempt);
                return;
            }
        }
        ...
        if (batchResponse != null && batchResponse.getFailureCount() == 0) {
            log.info("[{}] FCM 전송 모두 성공 (총 {}개)", Thread.currentThread().getName(), batchResponse.getSuccessCount());
            return;
        }
        ...
        if (retryCount + 1 >= MAX_RETRIES) {
            log.error("[{}] 최대 재시도 횟수가 초과되었습니다.", Thread.currentThread().getName());
            return;
        }
